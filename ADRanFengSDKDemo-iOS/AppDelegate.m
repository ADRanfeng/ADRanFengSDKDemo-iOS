...
@interface AppDelegate ()<ADRanFengSplashAdDelegate>
...
- (void)initTianSDK{
    ...
    [ADRanFengSDK setEnablePersonalInformation:YES];
    
    //初始化ADRanFengSDK
    [ADRanFengSDK initWithAppId:normal_appId completionBlock:^(NSError * _Nullable error) {
        if (error){
            NSLog(@"初始化失败%@",error);
        }else{
            NSLog(@"初始化成功");
